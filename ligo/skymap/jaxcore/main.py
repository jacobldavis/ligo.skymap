...
def bsm_jax(min_distance, max_distance, prior_distance_power, 
    ...
    # Initialize pixels
    # TODO: May have to change to np to allow pixels to be mutable
    order0 = 4
    nside = 1 << order0
    npix0 = 12 * nside * nside
    pixels = vmap(lambda ipix: [nest2uniq64(order0, ipix), [0, 0, 0]])(jnp.arange(npix0))
    ...
    accum = np.zeros(npix0, nifos)
    @jit
    def probability_map(i):
        bsm_pixel_jax(integrators, 1, pixels[i][0], pixels[i][1], 
                      gmst, nifos, nsamples, sample_rate, epochs, snrs, 
                      responses, locations, horizons, rescale_loglikelihood)
        vmap(lambda iifo: bsm_pixel_jax(integrators, 1, pixels[i][0], accum[i][iifo], gmst, 1, nsamples, 
                                        sample_rate, epochs[iifo], snrs[iifo], responses[iifo], locations[iifo], 
                                        horizons[iifo], rescale_loglikelihood))(jnp.arange(nifos))
